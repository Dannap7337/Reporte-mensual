...
@st.cache_data(ttl=300)
def load_escalados():
    path = "Datos escalados.xlsx"
    if os.path.exists(path):
        try:
            df = pd.read_excel(path)
            df.columns = [str(c).strip() for c in df.columns]
            col_fecha = next((c for c in df.columns if c.lower() == 'inicio'), None)
            if col_fecha:
...
# Aseguramos que 'pagina' siempre se defina para evitar NameError
...
if df is not None:
    ...
    # --- Lógica para Páginas Mensuales (1, 2, 3) ---
    if pagina in ["1. Generación", "2. Solución", "3. Contacto"]:
        ...
        if not meses_disp:
            ...
            if pagina == "1. Generación":
                ...
                st.markdown("---")
                st.subheader(f"⚠️ Top 5 Tickets cerrados con mayor demora")
                ...
                if not df_peores.empty:
                    st.table(df_peores[['N° TICKET', 'USUARIO', 'INICIO', 'FIN', 'DIAS', 'Detalle_Solucion']].style.format({"DIAS": "{:.0f}"}))
    ...
    # --- PÁGINA 4: RESUMEN ANUAL (ORIGINAL) ---
    ...
    # --- PÁGINA 5: ESCALADOS (NUEVA SECCIÓN) ---
        ...
        if df_esc is not None and 'dias_transcurridos' in df_esc.columns:
            col_motivo = next((c for c in df_esc.columns if c.lower() == 'motivo'), 'Motivo')
            ...
            with col1:
                ...
                if not df_fuera.empty:
                    fig1 = px.pie(df_fuera, names=col_motivo, hole=0.4, color_discrete_sequence=px.colors.qualitative.Reds_r)
                    st.plotly_chart(fig1, width='stretch')
                else: st.success("No hay tickets fuera de tiempo.")

            with col2:
                st.subheader("✅ En Tiempo (≤ 7 días)")
                df_dentro = df_esc[df_esc['dias_transcurridos'] <= 7]
                if not df_dentro.empty:
                    fig2 = px.pie(df_dentro, names=col_motivo, hole=0.4, color_discrete_sequence=px.colors.qualitative.Blues_r)
                    st.plotly_chart(fig2, width='stretch')
                else: st.info("No hay tickets escalados recientes.")
                
            with st.expander("Ver detalle de Escalados"):
                st.dataframe(df_esc)
        else:
            st.error("No se pudo cargar 'Datos escalados.xlsx' o falta la columna 'inicio'.")
